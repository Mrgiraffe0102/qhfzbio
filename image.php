...
<div id="container" class="single-attachment <?php QHFZBIO_get_layout_class(); ?>">
				...
				<div class="article-inner">
							...
							<?php MrGiraffe_post_meta_hook();
								...
								if ( $metadata ) {
									printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><i class="icon-image icon-metas" title="%1$s"></i> <a href="%2$s">%3$s &times; %4$s </a>%5$s</span>',
										esc_html_x( 'Full size', 'Used before full size attachment link.', 'QHFZBIO' ),
								...
								if ( ! empty( $post->post_parent ) ) :  ?>
									<span class="published-in">
										<i class="icon-book icon-metas" title="<?php esc_attr_e( 'Published in', 'QHFZBIO' ); ?>"></i>
					...
					<div class="entry-content" <?php MrGiraffe_schema_microdata( 'entry-content' ); ?>>
						...
						<div class="entry-attachment">
							<?php
							// actual attachment
							$image_size = apply_filters( 'QHFZBIO_attachment_size', 'large' );
					...
					<div id="nav-below" class="navigation image-navigation">
						<div class="nav-previous"><?php previous_image_link( false, '<i class="icon-angle-left"></i>' . __( "Previous image", "QHFZBIO" ) ); ?></div>
						<div class="nav-next"><?php next_image_link( false, __("Next image", "QHFZBIO") . '<i class="icon-angle-right"></i>' ); ?></div>
					</div><!-- #nav-below -->
