...
jQuery( document ).ready( function() {
	...
	jQuery( window ).on( 'scroll', function() {
		...
		if ( jQuery( this ).scrollTop() > 100 &&
			...
			jQuery( '.QHFZBIO-fixed-menu .site-header-bottom' ).addClass( 'header-fixed' );
		} else {
			jQuery( '.QHFZBIO-fixed-menu .site-header-bottom' ).removeClass( 'header-fixed' );
		}
...
function MrGiraffeAutoScroll(document, history, location) {
	...
	var anchorScrolls = {
		ANCHOR_REGEX: /^#[^ ]+$/,
		OFFSET_HEIGHT_PX: jQuery( '.QHFZBIO-fixed-menu #site-header-main' ).height(),
