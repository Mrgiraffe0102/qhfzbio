...
add_action( 'cryout_post_excerpt_hook', 'QHFZBIO_custom_excerpt_more', 10 );
...
add_action('cryout_headerimage_hook', 'QHFZBIO_meta_arrow');
...
function QHFZBIO_meta_infos() {
	...
	//add_action( 'cryout_featured_hook', 'QHFZBIO_posted_edit', 50 ); // Edit button

	if ( is_single() ) { // If single, metas are shown after the title

		add_action( 'cryout_post_meta_hook',	'QHFZBIO_posted_author_single', 10 );
		add_action( 'cryout_post_meta_hook',	'QHFZBIO_posted_date', 30 );
		add_action( 'cryout_post_title_hook',	'QHFZBIO_posted_category', 20 );
		add_action( 'cryout_post_utility_hook',	'QHFZBIO_posted_edit', 60 ); // Edit button
		add_action( 'cryout_post_utility_hook',	'QHFZBIO_posted_tags', 40 );

		if ( QHFZBIO_header_title_check() ) {
			add_action( 'cryout_headertitle_topmeta_hook',	'QHFZBIO_posted_category', 20 );
			add_action( 'cryout_headertitle_bottommeta_hook',	'QHFZBIO_posted_date', 20 );
			add_action( 'cryout_headertitle_bottommeta_hook',	'QHFZBIO_posted_author_single', 30 );
			//add_action( 'cryout_headertitle_bottommeta_hook',	'QHFZBIO_posted_edit', 50 ); // Edit button
		}

	} else { // if blog page, metas are shown at the top of the article

		add_action( 'cryout_post_thumbnail_hook', 'QHFZBIO_comments_on', 50 ); // Comments
		add_action( 'cryout_post_thumbnail_hook', 'QHFZBIO_posted_category', 20 );
		add_action( 'cryout_post_meta_hook', 'QHFZBIO_posted_author', 15 );
		add_action( 'cryout_post_meta_hook', 'QHFZBIO_posted_date', 40 );
		add_action( 'cryout_post_utility_hook',	'QHFZBIO_posted_tags', 30 );

	}

	add_action( 'cryout_singular_utility_hook',	'QHFZBIO_posted_edit', 40 ); // Edit button
	add_action( 'cryout_meta_format_hook', 'QHFZBIO_meta_format', 10 ); // Post format
	add_action( 'cryout_post_title_hook', 'QHFZBIO_meta_sticky', 9 ); // Sticky posts
...
function QHFZBIO_set_featured_srcset_picture() {
	...
	if ( function_exists('has_post_thumbnail') && has_post_thumbnail() && $options['theme_fpost']) {
		...
		$featured_image = cryout_post_first_image( $post->ID, 'QHFZBIO-featured' );
	...
	if ( ! empty( $featured_image[0] ) ) {
		...
		<div class="post-thumbnail-container" <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<picture class="responsive-featured-image">
				<source media="(max-width: 1152px)" sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['theme_magazinelayout'], $options['theme_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'QHFZBIO-featured-third' ) ); ?> 512w">
				<source media="(max-width: 800px)" sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['theme_magazinelayout'], $options['theme_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'QHFZBIO-featured-half' ) ); ?> 800w">
				<?php if ( cryout_on_landingpage() ) { ?><source sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['theme_magazinelayout'], $options['theme_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'QHFZBIO-featured-lp' ) ); ?> <?php printf( '%sw', absint( $featured_width ) ) ?>">
				<?php } ?>
				<img alt="<?php the_title_attribute();?>" <?php cryout_schema_microdata( 'url' ); ?> src="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'QHFZBIO-featured' ) ); ?>" width="<?php echo absint( $featured_image[1] ) ?>" height="<?php echo absint( $featured_image[2] ) ?>">
			</picture>
...
if ( cryout_get_option( 'theme_fpost' ) ) add_action( 'cryout_featured_hook', 'QHFZBIO_set_featured_srcset_picture' );
