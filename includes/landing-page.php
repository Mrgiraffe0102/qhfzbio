...
* slider output
*/
if ( ! function_exists( 'QHFZBIO_lpslider_output' ) ):
function QHFZBIO_lpslider_output( $data ){
extract($data);
if ( empty( $image ) && empty( $title ) && empty( $content ) && empty( $lpslidercta1text ) && empty( $lpslidercta2text ) ) return; ?>
	...
	<div class="lp-staticslider">
		...
		<?php echo QHFZBIO_meta_arrow(); ?>
	</div><!-- .lp-staticslider -->

<?php
} // QHFZBIO_lpslider_output()
endif;
...
function QHFZBIO_lpblocks( $sid = 1 ) {
	...
	?>
	<section id="lp-blocks<?php echo absint( $sid ) ?>" class="lp-blocks lp-blocks<?php echo absint( $sid ) ?> lp-blocks-rows-<?php echo esc_attr( apply_filters('QHFZBIO_blocks_perrow', $pagecount, $sid) ) ?>">
			...
			<?php foreach ( $pageids as $key => $pageid ) {
				...
				if ( intval( $pageid ) > 0 ) {
					...
					switch ( $blockscontent ) {
						...
						case '1': default: if (has_excerpt( $pageid )) $text = get_the_excerpt( $pageid ); else $text = QHFZBIO_custom_excerpt( apply_filters( 'the_content', get_post_field( 'post_content', $pageid ) ) ); break;
					};

					$iconid = preg_replace('/(\d)$/','icon$1', $key);

					$data[$count] = array(
						'title' => apply_filters('QHFZBIO_block_title', get_the_title( $pageid ), $pageid ),
						'text'	=> $text,
						'icon'	=> ( ( $icon[$iconid] != 'no-icon' ) ? $icon[$iconid] : '' ),
						'link'	=> apply_filters( 'QHFZBIO_block_url', get_permalink( $pageid ), $pageid ),
						'target' => apply_filters( 'QHFZBIO_block_target', '', $pageid ),
						'click'	=> $blocksclick,
						'id' 	=> $count,
						'readmore' => $blocksreadmore,
					);
					QHFZBIO_lpblock_output( $data[$count] );
					$count++;
				}
			} ?>
		</div>
 ...
 * blocks output
 */
if ( ! function_exists( 'QHFZBIO_lpblock_output' ) ):
function QHFZBIO_lpblock_output( $data ) { ?>
	<?php extract($data) ?>
		...
		<section id="lp-boxes-<?php echo absint( $sid ) ?>" class="lp-boxes lp-boxes-<?php echo absint( $sid ) ?> <?php  echo esc_attr( $animated_class ) ?> lp-boxes-rows-<?php echo absint( $options['theme_lpboxrow' . $sid] ); ?>">
    		...
    		<?php while ( $custom_query->have_posts() ) :
					...
					if ( cryout_has_manual_excerpt( $custom_query->post ) ) {
						...
						$excerpt = QHFZBIO_custom_excerpt( get_the_content(), $options['theme_lpboxlength' . $sid] );
					};
		            $box = array();
		            $box['colno'] = $box_counter++;
		            $box['counter'] = $options['theme_lpboxcount' . $sid];
		            $box['title'] = apply_filters( 'QHFZBIO_box_title', get_the_title(), get_the_ID() );
		            $box['content'] = $excerpt;
		            $box['image'] = wp_get_attachment_image( get_post_thumbnail_id( get_the_ID() ), 'QHFZBIO-lpbox-' . $sid );
					$box['link'] = apply_filters( 'QHFZBIO_box_url', get_permalink(), get_the_ID() );
					$box['readmore'] = apply_filters( 'QHFZBIO_box_readmore', $options['theme_lpboxreadmore' . $sid], get_the_ID() );
					$box['target'] = apply_filters( 'QHFZBIO_box_target', '', get_the_ID() );
					$box['image'] = apply_filters( 'QHFZBIO_preview_img_src', $box['image'] );

            		QHFZBIO_lpbox_output( $box );
        		endwhile; ?>
			</div>
		</section><!-- .lp-boxes -->
<?php endif;
 ...
 * boxes output
 */
if ( ! function_exists( 'QHFZBIO_lpbox_output' ) ):
function QHFZBIO_lpbox_output( $data ) {
...
function QHFZBIO_lptext( $what = 'one' ) {
	...
	if ( intval( $pageid ) > 0 ) {
		$page = get_post( $pageid );
		$data = array(
			'title' => apply_filters( 'QHFZBIO_text_title', get_the_title( $pageid ), $pageid ),
			'text'	=> apply_filters( 'the_content', get_post_field( 'post_content', $pageid ) ),
			'class' => apply_filters( 'QHFZBIO_text_class', '', $pageid ),
			'id' 	=> $what,
		);
		$data['image'] = wp_get_attachment_image( get_post_thumbnail_id( $pageid ), 'full' );
		QHFZBIO_lptext_output( $data );
	}
} // QHFZBIO_lptext()
endif;
 ...
 * text area output
 */
 if ( ! function_exists( 'QHFZBIO_lptext_output' ) ):
 function QHFZBIO_lptext_output( $data ){ ?>
